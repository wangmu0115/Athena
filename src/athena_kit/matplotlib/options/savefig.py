...
from athena_kit.matplotlib.types import BboxInches, ImageFormat


class PngExportStyle(_BaseOptions):
    compress_level: int = Field(
        6, ge=0, le=9, description="ZLIB 压缩级别：1=表示速度最快，9=表示压缩率最高，0=表示完全不压缩"
    )
...
class SaveFigureOptions(_BaseOptions):
    ...
    pad_inches: float | None = Field(
        None, ge=0, description="当 bbox_inches 设置为 `tight` 时，图形周围的内边距，单位 inch"
    )
