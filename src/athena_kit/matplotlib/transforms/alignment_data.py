...
from athena_kit.matplotlib.specs.plots import (
    BarPlot,
    LinePlot,
    PlotSpec,
)
from athena_kit.matplotlib.transforms.normalize_data import (
    NormalizedXYSeries,
...
def align_cartesian_plots_data(
    ...
    *,
    ...
    for plot in plots:
        match plot:
            case LinePlot():
                normalized = normalize_lineplot_data(
                    plot, coord, temporal_codec=temporal_codec, skip_invalid=skil_invalid
                )
            case BarPlot():
                normalized = normalize_barplot_data(
                    plot, coord, temporal_codec=temporal_codec, skip_invalid=skil_invalid
                )
            case _:
