...
class Lexer:
    def __init__(self, input: str):
        self.query_string = convert_bosun_query(input)
        ...
        while position < len(self.query_string):
            ...
            match ch:
                ...
                case ">" | "<" | "=" | "!":
                    ...
                    yield Token(BUILTIN_SYMBOLS[comp_op])
                case "&" | "|" as logic_op:
                    logic_op = _read_operator(self.query_string, position, logic_op)
                    position += 1
                    yield Token(BUILTIN_SYMBOLS[logic_op])
                case "," | "(" | ")":
                    yield Token(BUILTIN_SYMBOLS[ch])
                case '"':
                ...
                case _:
                    if _is_letter(ch):
                        ...
                        if iden in BUILTIN_FUNCS:
                            position += len(iden) - 1
                            yield Token(TokenType.BUILT_IN_FUNC, iden)
                        else:
